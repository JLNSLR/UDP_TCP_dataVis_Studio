...
def parse_floats(payload: bytes, prefer_binary=True):
    """
    First try binary little-endian float32 when length is multiple of 4.
    Fallback to ASCII (space/comma separated).
    """
...
class Studio(QtWidgets.QMainWindow):
    ...
    def _build_everything(self):
        ...
        # 2) Channels (from YAML, NO regex)
        self._build_channels_from_cfg()

        # 3) Controls (groups with Apply)
    ...
    def _build_channels_from_cfg(self):
        ...
        # Fallback: autogen ch0..ch{length-1} from first data source
    ...
    def _build_plots_from_cfg(self):
        # create plot widgets
