...
def _split_prefix_num(s: str):
    """Return (prefix, number). 'ch12' -> ('ch', 12). '12' -> ('', 12)."""
    s = str(s).strip()
    i = len(s)
...
def parse_floats(payload: bytes, prefer_binary=True):
    """Try float32 binary first, else ASCII floats."""
...
class Config:
    ...
    def reload(self):
        ...
        self.layout = cfg.get("layout", {})     # used for initial dock positions
...
class TcpClientSource(QtCore.QObject):
    """TCP client that connects to host:port and emits vector np.ndarray as frames arrive."""
    vector = QtCore.pyqtSignal(np.ndarray)

    def __init__(self, spec: dict, parent=None):
        super().__init__(parent)
        self.spec = spec
        self.sock = QtCore.QIODevice()  # placeholder to keep attribute around
        self.tcp = QtCore.QObject()     # ditto
        self._socket = QtWidgets.QApplication.instance()  # avoid linter warning
        # actual socket:
        self.socket = QtCore.QObject()  # shadow
        from PyQt5.QtNetwork import QTcpSocket
        self.socket = QTcpSocket(self)
        self.host = spec.get("host", "127.0.0.1")
        self.port = int(spec.get("port", 6000))
        ...
        self.socket.connected.connect(lambda: None)
    ...
    def _on_ready(self):
        ...
        if self.format == "float32_le_fixed":
            ...
            if self.length <= 0:
                return
            # peel off as many full frames as we have
...
class TcpServerSource(QtCore.QObject):
    """TCP server that listens on port and reads from the first connected client."""
    vector = QtCore.pyqtSignal(np.ndarray)
    ...
    def __init__(self, spec: dict, parent=None):
        ...
        if not self.server.listen(QHostAddress.AnyIPv4 if ip in ("0.0.0.0", "", None) else QHostAddress(ip), port):
    ...
    def _accept(self):
        ...
        if self.client:
            # optional: close previous and switch, or ignore. Here we switch.
            try:
...
class Studio(QtWidgets.QMainWindow):
    ...
    def _build_layout_controls(self):
        ...
        def save_state():
            ...
            # saveState returns a QByteArray; write as bytes
    ...
    def _build_plots_and_docks(self):
        ...
        for p in self.cfg.plots:
            title = p["title"]
            x_axis_cfg = p.get("x_axis", {})  # {mode: "time"|"samples", sample_rate_hz: 1000.0}
            ...
            if p.get("y_range"):
                lo, hi = p["y_range"]
                w.setYRange(lo, hi)
            w.setLabel("bottom", "Time (s)" if mode == "time" else "Samples")
        ...
        for r, row in enumerate(rows):
            ...
            for c, title in enumerate(row):
                ...
                if r == 0 and c == 0:
                    ...
                    # new row: place below the first dock of previous row if exists
    ...
    def _update_plots(self):
        for (title, name), curve in self.curves.items():
            ...
            # X mapping: samples or time (seconds)
            ...
            if mode == "time":
                ...
                if fs > 0:
                    ...
                    curve.setData(y)  # fall back to samples
            else:
                curve.setData(y)  # implicit x
